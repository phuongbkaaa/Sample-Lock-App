...
public class absolutelyNeed extends AppCompatActivity implements ComponentCallbacks2 {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        intent = getIntent();
        if (intent.getStringExtra(timePickerActivity.UNIQUE_ID).equals("timePickerActivity")) {
        ...
        /** FROM APP LIST */
        if (intent.getStringExtra(timePickerActivity.UNIQUE_ID).equals("AllAppsList")) {
        ...
        if(intent.getStringExtra(timePickerActivity.UNIQUE_ID).equals("OpenSettingsService")) {
            stopService(intentForOpenSettingsService);
        }

            /** OPENING FROM SERVICE*/

            if(intent.getStringExtra(timePickerActivity.UNIQUE_ID).equals("StartPlanService")){
    ...
    private class registerTime extends AsyncTask<String, Void, String> {
        ...
        @Override
        protected String doInBackground(String... strings) {
            timeOffHours = intent.getIntExtra(timePickerActivity.TIME_OFF_UNTIL_HOURS, 0);
            timeOffMinutes = intent.getIntExtra(timePickerActivity.TIME_OFF_UNTIL_MINUTES, 0);
            START_TIME_IN_MILLIS = intent.getIntExtra(timePickerActivity.TOTAL_OFF_TIME, 0);
